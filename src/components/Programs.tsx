...
export default function Programs() {
  ...
  const filtered = programs.filter((p) => activeFilter === 'All' || p.level === activeFilter)

  return (
    <section id="programs" ref={ref} className="py-28 relative"
      style={{ background: 'linear-gradient(180deg, #0a0f2e 0%, #0d1540 50%, #0a0f2e 100%)' }}>
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="text-center mb-16">
          <div className="animate-on-scroll inline-flex items-center gap-2 mb-6">
            ...
            <span className="text-gold-400 font-mono text-sm tracking-widest uppercase">Academic Programs</span>
            <div className="h-px w-12 bg-gold-400" />
          </div>
          <h2 className="animate-on-scroll font-display font-black text-4xl md:text-5xl text-cream mb-4" style={{ transitionDelay: '0.1s' }}>
            Build Your Future,{' '}
            <span className="gold-text">Your Way</span>
          </h2>
          <p className="animate-on-scroll text-cream/50 text-lg max-w-2xl mx-auto" style={{ transitionDelay: '0.2s' }}>
            From undergraduate foundations to postgraduate specialisations — find the programme that launches your ambition.
          </p>

          {/* Filter tabs */}
          <div className="animate-on-scroll flex justify-center gap-3 mt-8" style={{ transitionDelay: '0.3s' }}>
          ...
          {filtered.map((prog, i) => (
            ...
            >
              ...
              <div className="flex items-start justify-between mb-4">
                <div>
                  <span className="font-mono text-xs tracking-widest text-cream/40 uppercase">{prog.level} · {prog.duration}</span>
                  <h3 className="font-display font-black text-2xl text-cream mt-1 group-hover:text-gold-400 transition-colors">
                    {prog.name}
                  </h3>
                </div>
                <div className="w-3 h-3 rounded-full mt-2 ring-2 ring-offset-2 ring-offset-navy-800 transition-transform group-hover:scale-125"
                  style={{ background: prog.color, ringColor: prog.color }} />
              </div>

              <p className="text-cream/50 text-sm mb-4 leading-relaxed">{prog.desc}</p>
              ...
              <div className="flex items-center justify-between pt-4 border-t border-cream/5">
                <span className="text-gold-400 font-semibold text-sm">{prog.fee}</span>
                <a href="#cta" className="text-cream/40 text-xs hover:text-gold-400 transition-colors font-mono">
                  Apply →
                </a>
              </div>
            </div>
